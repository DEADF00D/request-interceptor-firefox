...
handleBeforeRequest(details) {
    ...
    if (rule) {
        ...
        if (Core.isInterceptEnabled) {
            ...
            filter.ondata = e => {
                filter.write(e.data);
                if(!responseBody[newRequestId]){
                    responseBody[newRequestId] = '';
                }
                responseBody[newRequestId] += decoder.decode(e.data);
            };
            filter.onstop = e => {
                filter.close();

                console.log("responseBody:", responseBody[newRequestId]);

                Storage.addRequest({
                    ...details,
                    responseBody: responseBody[newRequestId],
                    timeStamp: Date.now(),
